...
from ni_collator import DataCollatorForIE
...
class DataTrainingArguments:
    ...
    data_dir: str = field(
        default="/root/IE_LLM/train_data/", metadata={"help": "The directory for saving the NaturalInstructions train/dev/test splits."}
    )
    task_dir: str = field(
    ...
    max_num_instances_per_eval_task: int = field(
        default=None, metadata={"help": "The maximum number of instances we will consider for each validation/test task."}
    )
    ...
    add_correction: Optional[bool] = field(
        default=True,
        metadata={"help": "whether to add explanation for both the postive examples and negtive examples."}
    )
    try_all: Optional[bool] = field(
        default=False,
...
@dataclass
class IETrainingArguments(Seq2SeqTrainingArguments):
    ...
    do_train: bool = field(default=True,metadata={"help": "Path to pretrained model or model identifier from huggingface.co/models"})
    do_eval: bool = field(default=True,metadata={"help": "Path to pretrained model or model identifier from huggingface.co/models"})
    predict_with_generate: bool = field(default=True,metadata={"help": "Path to pretrained model or model identifier from huggingface.co/models"})
    generation_max_length: Optional[int] = field(default=128,metadata={"help": "Path to pretrained model or model identifier from huggingface.co/models"})
    ...
    _n_gpu : Optional[int] = field(default=1)
...
def main():
    ...
    parser = HfArgumentParser((ModelArguments, DataTrainingArguments, IETrainingArguments))
    ...
    if training_args.do_train:
        ...
        if data_args.max_train_samples is not None:
            train_dataset_ind = random.sample(range(len(train_dataset)),data_args.max_train_samples)
            train_dataset = train_dataset.select(train_dataset_ind)
    ...
    data_collator = DataCollatorForIE(
        tokenizer,
        model=model,
        ...
        add_type_definition=data_args.add_task_definition,
        add_correction = data_args.add_correction,
    ...
    if training_args.do_train:
        ...
        train_result = trainer.train(resume_from_checkpoint=checkpoint,max_length = max_length,num_beams = num_beams)
