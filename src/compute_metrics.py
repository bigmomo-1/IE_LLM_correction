...
def triple_match(predictions, ground_truths):
    TP = 0
    pattern = re.compile(r'({.*?})')
    recall_sum = 0
    precision_sum = 0
    for pred,truth in zip(predictions,ground_truths):
        model_pred = re.findall(pattern, pred)
        ground_truth = re.findall(pattern,truth)
        
        recall_sum += len(ground_truth)
...
def metric_max_over_ground_truths(metric_fn, prediction, ground_truth, xlingual=False):
...
def compute_metrics(predictions, references, xlingual=False):
    ...
    for pred, gold in zip(predictions, references):
        ...
        exact_match += metric_max_over_ground_truths(
            exact_match_score, prediction=pred, ground_truth=gold, xlingual=xlingual
        )
        rouge1 += metric_max_over_ground_truths(
            rouge1_score, prediction=pred, ground_truth=gold, xlingual=xlingual
        )
        rougeL += metric_max_over_ground_truths(
            rougeL_score, prediction=pred, ground_truth=gold, xlingual=xlingual
        )
    triple_metric = triple_match(predictions,references)
    exact_match = 100.0 * exact_match / len(references)
    rouge1 = 100.0 * rouge1 / len(references)
    rougeL = 100.0 * rougeL / len(references)
    metrics = {"exact_match": exact_match, "rouge1": rouge1, "rougeL": rougeL,"triple":triple_metric}
